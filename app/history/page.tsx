  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "#F5F0E8" }}>
      <header className="sticky top-0 z-10 px-5 py-4 flex items-center gap-3"
        ...
        <h1 className="text-base font-semibold" style={{ color: "#2D5A27" }}>利用履歴</h1>
      </header>
        ...
        {checkins.length > 0 ? (
          <div className="space-y-3">
            {checkins.map((checkin) => {
              ...
              return (
                <div key={checkin.id} className="flex items-center gap-4 p-4 rounded-2xl"
                  ...
                  <div className="flex-shrink-0 w-12 h-12 rounded-xl flex flex-col items-center justify-center"
                    style={{ backgroundColor: "#E8E4DC" }}>
                    <span className="text-xs font-medium" style={{ color: "#8B6914" }}>{dt.weekday}</span>
                    <span className="text-lg font-bold leading-none" style={{ color: "#2D5A27" }}>{dt.day}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium" style={{ color: "#4A4A4A" }}>TORCH 4h PASS 利用</p>
                    <p className="text-xs mt-0.5" style={{ color: "#8B6914" }}>{dt.date} {dt.time}</p>
                  </div>
          ...
          <div className="text-center py-16">
            ...
            <p className="font-medium" style={{ color: "#4A4A4A" }}>まだ利用履歴がありません</p>
            <p className="text-sm mt-1" style={{ color: "#8B6914" }}>チェックイン後に記録されます</p>
          </div>
        )}
      </main>
    </div>
