...
export default function LoginPage() {
  ...
  async function handleMagicLink(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email,
      options: { emailRedirectTo: `${window.location.origin}/harunaapp/auth/callback` },
    });
  ...
  async function handleGoogle() {
    ...
    await supabase.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: `${window.location.origin}/harunaapp/auth/callback` },
    });
  }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-12"
      ...
      <div className="mb-10 text-center">
        ...
        <h1 className="text-2xl font-semibold tracking-wide" style={{ color: "#2D5A27" }}>TORCH Pass</h1>
        <p className="mt-1 text-sm" style={{ color: "#8B6914" }}>コワーキング回数券</p>
      </div>
      ...
      <div className="w-full max-w-sm">
        {sent ? (
          <div className="text-center p-8 rounded-2xl" style={{ backgroundColor: "#FDFAF5" }}>
            ...
            <h2 className="text-lg font-medium mb-2" style={{ color: "#2D5A27" }}>メールを送信しました</h2>
            <p className="text-sm" style={{ color: "#4A4A4A" }}>
              <strong>{email}</strong> 宛にログインリンクを送りました。メールをご確認ください。
            </p>
            <button onClick={() => { setSent(false); setEmail(""); }}
              className="mt-6 text-sm underline" style={{ color: "#8B6914" }}>
              別のメールで試す
            </button>
          </div>
        ) : (
          <div className="p-8 rounded-2xl shadow-sm" style={{ backgroundColor: "#FDFAF5" }}>
            <button onClick={handleGoogle} disabled={loading}
              ...
              style={{ borderColor: "#E8E4DC", color: "#4A4A4A", backgroundColor: "white" }}>
            ...
            <form onSubmit={handleMagicLink} className="space-y-4">
              <div>
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                  ...
                  style={{ borderColor: "#E8E4DC", backgroundColor: "white", color: "#4A4A4A" }} />
              </div>
